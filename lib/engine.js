...
export function createStore() {
  return {
    ...
    slots: structuredClone(slotCatalog),
    fairness: {
...
export function spin(store, { playerId, slotId, lineBet, clientSeed = 'mogambl-default' }) {
  const player = getPlayer(store, playerId);
  ...
  const grid = Array.from({ length: 3 }, () => Array.from({ length: 5 }, () => weightedRandom(rng)));
